...
TTO = Namespace("https://materialdigital.github.io/application-ontologies/tto/#/")#TTO ontology
g.bind("tto", TTO)  #Base
...
g.add((thicknessIRI_scalar_value, PMD.PMD_0000006, Literal(thickness, datatype=XSD.float)))
g.add((thicknessIRI_scalar_value, PMD.PMD_0060001, thicknessIRI_quality))# specifies thickness
g.add((thicknessIRI_scalar_value, OBO.IAO_0000136, testPieceIRI))# is about test piece

with open(csv_filename, mode="w", newline="") as f:
    writer = csv.writer(f) , writer.writerow(["Force(N)", "Elongation(mm)"])# header
